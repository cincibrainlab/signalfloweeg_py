...
from bottle import route, run, request, error, response, HTTPError, static_file, post
...
<body>
    ...
    <div id="content" style="width: 800px; margin: 0 auto;">
        <h2>SignalFlow EEG Analysis Portal</h2><br>
        Upload new files:
        ...
        <form action="/submit" method="post">
            <div class="form-group">
                <label for="eegType">EEG Type:</label>
                <select class="form-control" id="eegType" name="eegType">
                    <option value="human128">Human: SET Format (EGI 128)</option>
                    <option value="mouse30">Mouse: SET Format (30 Channel)</option>
                </select>
            </div>
            <div class="form-group">
                <label for="analysisType">Analysis Type:</label>
                <select class="form-control" id="analysisType" name="analysisType">
                    <option value="connectivity">Connectivity: SOH, ISPC, DWPLI</option>
                    <option value="power">Power Analysis: PSD, PWPLI</option>
                    <option value="fooof">Spectral Parameterization (FOOOF)</option>
                </select>
            </div>
            <div class="form-group">
                <label for="email">Email Address to Send Results:</label>
                <input type="email" class="form-control" id="email" name="email" placeholder="Enter your email" value="test@example.com">
            </div>
            <button type="submit" class="btn btn-success" id="runJobButton" disabled>Run SignalFlow Job</button>
        </form>
        ...
        <script type="application/javascript">
            function clearCookies() {{
                ...
                document.getElementById("runJobButton").disabled = true; // Disable the run job button when clearing cookies
            }}
            ...
            function saveCookie(new_file) {{
                    ...
                    if (all_files.length > 0) {{
                        document.getElementById("runJobButton").disabled = false; // Enable the run job button if there are files
                    }}
            }}
...
@post('/submit')
def submit():
    eeg_type = request.forms.get('eegType')
    analysis_type = request.forms.get('analysisType')
    email = request.forms.get('email')
    # Retrieve the list of uploaded files for the submission
    uploaded_files = [file.name for file in storage_path.iterdir() if file.is_file()]
    submission_data = {
        'eeg_type': eeg_type,
        'analysis_type': analysis_type,
        'email': email,
        'uploaded_files': uploaded_files,  # Add the list of uploaded files to the submission data
    }
    submission_filename = storage_path / f"submission_{uuid.uuid4()}.yaml"
    with open(submission_filename, 'w') as file:
        yaml.dump(submission_data, file)
    return f"Submission saved: {submission_filename}"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Timeout (ms) for each chunk upload",
    )
